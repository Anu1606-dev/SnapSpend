...
import {
  getDaysInMonth, getDaysElapsedInMonth, getUsualMonthlyAverage, getBudgetMood,
} from '../utils/budgetCalculations'

const MOOD_STYLES = {
  happy: {
    Icon: Smile,
    iconBg: 'bg-emerald-500 dark:bg-gradient-to-br dark:from-mint dark:to-electric',
    text: 'text-emerald-700 dark:text-mint',
    heroBg: 'bg-gradient-to-br from-emerald-50 to-white dark:from-mint/15 dark:to-surface',
    ring: 'ring-emerald-100 dark:ring-mint/30',
  },
  thinking: {
    Icon: Brain,
    iconBg: 'bg-blue-500 dark:bg-gradient-to-br dark:from-electric dark:to-violet',
    text: 'text-blue-700 dark:text-electric',
    heroBg: 'bg-gradient-to-br from-blue-50 to-white dark:from-electric/15 dark:to-surface',
    ring: 'ring-blue-100 dark:ring-electric/30',
  },
  sad: {
    Icon: Frown,
    iconBg: 'bg-amber-500 dark:bg-gradient-to-br dark:from-sun dark:to-flame',
    text: 'text-amber-700 dark:text-sun',
    heroBg: 'bg-gradient-to-br from-amber-50 to-white dark:from-sun/15 dark:to-surface',
    ring: 'ring-amber-100 dark:ring-sun/30',
  },
  tensed: {
    Icon: AlertTriangle,
    iconBg: 'bg-red-500 dark:bg-gradient-to-br dark:from-rose dark:to-magenta',
    text: 'text-red-700 dark:text-rose',
    heroBg: 'bg-gradient-to-br from-red-50 to-white dark:from-rose/15 dark:to-surface',
    ring: 'ring-red-100 dark:ring-rose/30',
  },
}

const fieldLabel = 'block text-sm font-medium text-slate-700 dark:text-cloud mb-1.5'
const fieldInput = 'w-full pl-8 pr-4 py-2.5 border border-slate-200 dark:border-edge dark:bg-inset dark:text-white rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-shadow'

function StatCard({ icon: Icon, color, label, value, sub, delay = 0 }) {
  const colorMap = {
    blue: 'bg-blue-50 text-blue-600 dark:bg-gradient-to-br dark:from-electric dark:to-electric-deep dark:text-white',
    violet: 'bg-violet-50 text-violet-600 dark:bg-gradient-to-br dark:from-violet dark:to-magenta dark:text-white',
    emerald: 'bg-emerald-50 text-emerald-600 dark:bg-gradient-to-br dark:from-mint dark:to-electric dark:text-white',
    red: 'bg-red-50 text-red-600 dark:bg-gradient-to-br dark:from-rose dark:to-magenta dark:text-white',
    amber: 'bg-amber-50 text-amber-600 dark:bg-gradient-to-br dark:from-sun dark:to-flame dark:text-white',
    teal: 'bg-teal-50 text-teal-600 dark:bg-gradient-to-br dark:from-mint dark:to-mint-deep dark:text-white',
  }
  return (
    <div className="bg-white dark:bg-surface rounded-2xl shadow-sm p-4 card-hover animate-fade-in-up" style={{ animationDelay: `${delay}ms` }}>
      <div className={`w-8 h-8 rounded-lg flex items-center justify-center mb-2 ${colorMap[color]}`}>
        <Icon size={16} />
      </div>
      <p className="text-xs text-slate-500 dark:text-cloud mb-0.5">{label}</p>
      <p className="text-lg font-bold text-slate-900 dark:text-white">{value}</p>
      {sub && <p className="text-xs text-slate-400 dark:text-smoke mt-0.5">{sub}</p>}
    </div>
  )
}
  ...
  return (
    <div className="max-w-3xl mx-auto p-6 md:p-8">
      <div className="mb-6 flex items-center justify-between">
        <div>
          <h1 className="text-2xl md:text-3xl font-bold text-slate-900 dark:text-white flex items-center gap-2">
            <PiggyBank size={28} className="text-emerald-500 dark:text-mint" />
            Budget & Savings
          </h1>
          <p className="text-slate-500 dark:text-cloud text-sm mt-1">Set a target, track your pace, know where you stand.</p>
        </div>
        {hasBudget && !editing && (
          <button
            onClick={() => {
      ...
      {fetchStatus === 'succeeded' && showSetupForm && (
        <div className="bg-white dark:bg-surface rounded-2xl shadow-sm p-6 md:p-8 animate-fade-in-up">
          <h2 className="font-semibold text-slate-900 dark:text-white mb-1">{hasBudget ? 'Update your numbers' : "Let's set up your budget"}</h2>
          <p className="text-sm text-slate-500 dark:text-cloud mb-6">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            {error && <p className="text-sm text-red-600 dark:text-rose bg-red-50 dark:bg-rose/10 border border-red-100 dark:border-rose/20 px-3 py-2 rounded-xl">{error}</p>}
            <div className="flex gap-2">
              ...
              {hasBudget && (
                <button type="button" onClick={() => setEditing(false)} className="btn-secondary px-6 py-2.5 text-sm">
                  Cancel
                </button>
              )}
            </div>
          ...
          <div className={`rounded-2xl shadow-sm p-6 md:p-8 ${moodStyle.heroBg} ring-1 ${moodStyle.ring} animate-fade-in-up`}>
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-slate-800 dark:text-slate-100 font-medium mt-0.5">{mood.message}</p>
              </div>
            </div>
          </div>

          <div className="bg-white dark:bg-surface rounded-2xl shadow-sm p-5 md:p-6 animate-fade-in-up" style={{ animationDelay: '60ms' }}>
            <div className="flex items-center gap-2 mb-4">
              <Target size={18} className="text-slate-400 dark:text-smoke" />
              <h2 className="font-semibold text-slate-900 dark:text-white">Budget Pace</h2>
            </div>
            <div className="relative h-3 bg-slate-100 dark:bg-inset rounded-full overflow-hidden">
            ...
            <div className="flex justify-between text-xs text-slate-400 dark:text-smoke mt-2">
